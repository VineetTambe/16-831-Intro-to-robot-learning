...
class RL_Trainer(object):
    ...
    def dump_density_graphs(self, itr):
        ...
        shape = (ii.shape[0], ii.shape[1], density.size // ii.size)
        density = density.reshape(shape)
        density = np.mean(density, axis=2)
        # density = np.mean(density, axis=1).squeeze()
        ...
        shape = (ii.shape[0], ii.shape[1], exploitation_values.size // ii.size)
        exploitation_values = exploitation_values.reshape(shape)
        exploitation_values = np.mean(exploitation_values, axis=2)
        # exploitation_values = np.mean(exploitation_values, axis=1).squeeze()
        ...
        shape = (ii.shape[0], ii.shape[1], exploration_values.size // ii.size)
        exploration_values = exploration_values.reshape(shape)
        exploration_values = np.mean(exploration_values, axis=2)
        # exploitation_values = np.mean(exploration_values, axis=1).squeeze()
